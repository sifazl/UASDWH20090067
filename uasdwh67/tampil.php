<?php include "_header.php"; ?>
	...
	<table class="data" border="1" style="border-collapse: collapse; margin-bottom: 5px;">
		...
		<?php
		$sql = mysqli_query($con, "SELECT * FROM prediksi") or die (mysqli_error($con));
		if (mysqli_num_rows($sql) > 0) {
			...
			while ($data = mysqli_fetch_array($sql)) {
				$jumlah_x += $x;
				$jumlah_y += $data['Customer'];
				$jumlah_xx += ($x * $x);
				$jumlah_xy += ($x * $data['Customer']); 
				?>
				<tr>
					<td><?=$x+1;?>.</td>
					<td><?=$data['Tahun'];?></td>
					...
					<td align="center"><?=$x * $data['Customer'];?></td>
				</tr>
				<?php
				$x++;
			}
	...
	if(isset($_POST['prediksi'])) {
		...
		?>
		<div>
			Prediksi Customer untuk <?=$tahun;?> tahun berikutnya adalah <?=$prediksi;?>
		</div>
	<?php
	}
	?>
